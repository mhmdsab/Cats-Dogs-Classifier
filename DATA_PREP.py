...
def prepare_data(data_directory_path):
    ...
    for file in file_directory:
        if file == 'cats':
            ...
            cats_train_arrays_flipped_h = [cv2.flip(img,1) for img in cats_train_arrays]
            cats_train_arrays_flipped_v = [cv2.flip(img,0) for img in cats_train_arrays]
            cats_train_arrays_flipped_hv = [cv2.flip(img,-1) for img in cats_train_arrays]
            cats_train_arrays.extend(cats_train_arrays_flipped_h)
            cats_train_arrays.extend(cats_train_arrays_flipped_v)
            cats_train_arrays.extend(cats_train_arrays_flipped_hv)
            ...
            dogs_train_arrays_flipped_h = [cv2.flip(img,1) for img in dogs_train_arrays]
            dogs_train_arrays_flipped_v = [cv2.flip(img,0) for img in dogs_train_arrays]
            dogs_train_arrays_flipped_hv = [cv2.flip(img,-1) for img in dogs_train_arrays]
            dogs_train_arrays.extend(dogs_train_arrays_flipped_h)
            dogs_train_arrays.extend(dogs_train_arrays_flipped_v)
            dogs_train_arrays.extend(dogs_train_arrays_flipped_hv)
            dogs_train_labels = np.ones(shape = len(dogs_train_arrays),dtype = 'float32')
            print('dogs processing finished')
    print('step 1: Done!')
    
    
    print('step 2: shuffling the data before feeding them to the network')
    ...
    merged_train_dogs_cats = ((merged_train_dogs_cats-255)/255).astype('float32')#/255
    merged_labels_dogs_cats = merged_labels_dogs_cats[permutations]
    merged_labels_dogs_cats = merged_labels_dogs_cats.reshape(-1, 1)
    print('step 2: Done!')
    

    print('step 3: splitting the data into train and cv sets')

    
    x_train,x_cv,y_train,y_cv = train_test_split(merged_train_dogs_cats,merged_labels_dogs_cats,test_size = 0.008)
    print('step 3: Done!')
